...
public class PiezaFacadeREST extends AbstractFacade<Pieza> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Pieza entity) {
        super.edit(entity);
    }
    ...
    public Pieza find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Pieza> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
